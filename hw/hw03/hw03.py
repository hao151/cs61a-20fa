...
def balanced(m):
    ...
    if is_mobile(m):
        if not balanced(left(m)) or not balanced(right(m)):
            return False
        ...
        l_torque = left_length * left_weight
        r_torque = right_length * right_weight
...
def preorder(t):
    ...
    """
    if is_tree(t):
        return [label(t)] + [preorder(branch) for branch in branches(t)]    #False
    """
...
def has_path(t, word):
    ...
    "*** YOUR CODE HERE ***"
    def trie(t, idx):
        if is_tree(t) and label(t) == word[idx]:
            if idx == len(word) - 1:
                return True
            for branch in branches(t):
                if trie(branch, idx + 1):
                    return True
        return False
    return trie(t, 0)
...
def div_interval(x, y):
    ...
    assert  lower_bound(reciprocal_y) * upper_bound(reciprocal_y) > 0, "AssertionError"
