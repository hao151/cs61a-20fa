...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        #shceme_apply will do "validate_procedure"
        operator = scheme_eval(first, env)   #get the operator by recursively call "Scheme_eval"
        argument = rest.map(lambda expr: scheme_eval(expr, env)) #recusively get the expression tree
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        function = do_lambda_form(Pair(formals, body), env)
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        ...
        if is_false_primitive(return_value):
            return return_value
        expressions = expressions.rest
    return return_value
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while expressions != nil:
        return_value = scheme_eval(expressions.first, env)
        if is_true_primitive(return_value):
            return return_value
        expressions = expressions.rest
    return return_value
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            if clause.rest == nil:
                return test
            if isinstance(clause.rest, Pair):
