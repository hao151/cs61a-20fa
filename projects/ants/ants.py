...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self
        elif self.can_contain(place.ant):     #self是容器
        	self.contain_ant(place.ant)       #给蚂蚁装到容器里
        	place.ant = self                  #令地上的蚂蚁具有容器属性（地上的宠物是容器）, 最后将容器放到地上（容器的地是这块地）相当于双向绑定
        
        elif place.ant.can_contain(self):     #self是蚂蚁
        	place.ant.contain_ant(self)       #下一步可以直接把蚂蚁放到地上，因为地上已经有了容器 
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        cnt = 0
        while cur_place != beehive:            #从min_range开始计算
            if cnt == min_range:
                break
            cnt += 1
            cur_place = cur_place.entrance

        while len(cur_place.bees) == 0 and cur_place != beehive:
            cur_place = cur_place.entrance
            cnt += 1

        if cur_place == beehive or cnt > max_range:
            return None
        else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        def reflect(damage):
            bee_armor = place.bees[:]
            for i in bee_armor:
                idx = place.bees.index(i)
                bees = place.bees[idx]
    ...
    def action(self, gamestate):
        ...
        if not self.is_true_queen:
            ...
            while place != None:
                ...
                if place.ant.is_double == False:
                    place.ant.damage *= 2
                    place.ant.is_double = True
                place = place.exit
        # END Problem EC
...
class ContainerAnt(Ant):     #先令容器具有里面蚂蚁的属性，等容器破碎后再让该地方的蚂蚁等于原先容器里面的蚂蚁。
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target and target.is_scared == False:
            apply_status(make_scary, target, 2)
            is_scared = true
        # END Problem Optional 4
