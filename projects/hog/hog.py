...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    if player_score and opponent_score: #both numbers are positive
        if gcd(player_score, opponent_score) >= 10: #辗转相除法
            return True
    return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            if extra_turn(score0, score1) == False:   #change player until extra_turn == False
                who = other(who)
        else:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 1:
            ...
            now_score = score0
        if now_score - last_score > running_high:
            more = now_score - last_score
            print("%d point(s)! The most yet for Player %d" % (more, who))
            return announce_highest(who, now_score, more)
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    # Question 8 > Suite 2 > Case 2 here need to pay attention
    #奇数次选择两个色子时点数均为 3, 1 由于规则说只要出现1，则这次的得分就是1
    #所以最后结果应该是：(1 + 11) * 500 / 1000 = 6.0

    "*** YOUR CODE HERE ***"
    #英语不好，开始理解错了....其实就是求同一个函数n次返回值的平均值 由于test_dice的原因导致每次返回值不一样。
    def aver(*args):
        res = 0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_aver_nums, max_aver_score = 0, 0
    # question asked is nums[1, 10] not include **0**
    for i in range(1, 11):
        cur_score = make_averaged(roll_dice, trials_count)(i, dice)
        if cur_score > max_aver_score:
            max_aver_nums, max_aver_score = i, cur_score
    return max_aver_nums
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    # 空掷会多一轮 或者 空掷会达到cutoff的话就空掷， 否则就返回nums_rolls
    virtual_score = free_bacon(opponent_score)
    if extra_turn(score + virtual_score, opponent_score):
        return 0
