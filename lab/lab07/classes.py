...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        tmp = opponent.deck.cards[:]
        for i in range(len(opponent.deck.cards)):
            if opponent.deck.cards[i].attack == attack and opponent.deck.cards[i].defense == defense:
                tmp.remove(opponent.deck.cards[i])
        opponent.deck.cards = tmp
