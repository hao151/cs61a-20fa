...
def prune_min(t):
    ...
    for branch in t.branches:
        child += [branch]
        prune_min(branch)
    if child[0].label > child[1].label:
        t.branches = [child[1]]
    else:
        t.branches = [child[0]]
...
class Check(object):
    "*** YOUR CODE HERE ***"
    def __init__(self, belong_to, check_amount):
        self.amount = check_amount
        self.deposited = False
        self.payable_to = belong_to
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            edits, cost = code_char + possibility[0], possibility[1]
            possibilities.append((edits, cost))
        # Insert
        possibility = helper_align(skeleton_idx, code_idx + 1)
        edits, cost = "+[" + code_char + "]" + possibility[0], possibility[1] + 1
        possibilities.append((edits, cost))
        # Delete
        possibility = helper_align(skeleton_idx + 1, code_idx)
        edits, cost = "-[" + skel_char + "]" + possibility[0], possibility[1] + 1
        possibilities.append((edits, cost))
...
def filterl(lst, pred):
    ...
    """
    "*** YOUR CODE HERE ***"
    def fn(lst_ans, number):
        if pred(number) == True:
            if lst_ans == Link.empty:
                lst_ans = Link(number)
            else:
                cur = lst_ans
                while cur.rest != Link.empty:
                    cur = cur.rest
                cur.rest = Link(number)
        return lst_ans
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda list, num: Link(num, list), Link.empty)
